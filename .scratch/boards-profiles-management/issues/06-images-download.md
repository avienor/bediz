...
**Review gate:** Review the fixed diff against spec §15, §15.1, and this ticket. Independently check that a failed download leaves no partial file at the target, that an existing file is never overwritten, including by a race between check and rename where the platform allows it, and that the size limit holds. Overwriting a file or leaving a partial output blocks acceptance.

**Escalate when:** The 6.14.1 full-image endpoint needs something other than a plain GET, or an atomic no-overwrite rename is impossible on a release target (Linux, Windows, macOS).
...
- Bediz writes to a temporary file in the target directory and then renames it into place. It removes the temporary file on any failure.

...
4. **Cancellation is per item.** `queue cancel ITEM_ID` cancels the named item together with the non-terminal items InvokeAI links to it in the same workflow-call chain. InvokeAI 6.14.1 applies cancellation to the whole chain but leaves completed, failed, and already canceled items unchanged. Bediz's own generate and upscale batches create no chains. Batch cancellation is deferred because every receipt already lists item identifiers.

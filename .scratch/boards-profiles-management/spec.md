...
4. **Cancellation is per item.** `queue cancel ITEM_ID` cancels the non-terminal items in the named item's workflow-call chain, and the named item itself counts as part of that chain. InvokeAI 6.14.1 leaves completed, failed, and already canceled items unchanged. Bediz's own generate and upscale batches create no chains. Batch cancellation is deferred because every receipt already lists item identifiers.

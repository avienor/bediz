...
**2026-09-24, `v1.0.0-rc.1` re-released:** at the user's request, the first `v1.0.0-rc.1` release and tag (commit 85af9f3, workflow run 36019536669) were deleted, because they shipped the skill text from before the review fixes. The tag now points to 4f35c8d, the merge of PR #12. `go run ./tools/release -check v1.0.0-rc.1` passed there. Workflow run 36024446114 published a non-draft prerelease with the three archives and `SHA256SUMS`, and `sha256sum -c SHA256SUMS` reports `OK` for all three archives. The Linux binary reports `v1.0.0-rc.1`, commit `4f35c8db…`, and date `2026-09-24T15:58:04Z`. The skill at the tag reads the failed `doctor` report from `error.details.report`. Anyone who installed the earlier rc.1 should reinstall it; the note above about rc.1 carrying the old skill text no longer applies.

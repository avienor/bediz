...
- The main model must have base `flux`, type `main`, and InvokeAI variant `dev` or `schnell`. On the stock InvokeAI 6.14.1 loader, the supported formats are `checkpoint`, `bnb_quantized_nf4b`, and `gguf_quantized`. Variant `dev_fill`, a missing or unknown variant, and every other format, including `sdnq_quantized` and ordinary `diffusers`, return `unsupported_capability`. FLUX.1 Krea dev and Kontext dev models report variant `dev` and are accepted for text-to-image when their format is supported. The installed 6.14.1 `flux_model_loader` accepts checkpoint-based configs and its SDNQ Diffusers config, but rejects the ordinary FLUX Diffusers config; SDNQ stays outside this slice.

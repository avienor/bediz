...
Upgrading Go for releases is an explicit `go.mod` change. A `go install` user with an older local Go downloads the pinned toolchain through the default `GOTOOLCHAIN=auto` setting. A newer local toolchain builds normally. The release command must run on a host that matches a release target, because it runs the host's binary from the finished archive to check the reported version, commit, and date. The tag-triggered publication workflow runs the same command, so published archives and local rebuilds come from one code path.

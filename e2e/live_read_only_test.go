...
func TestLiveGate(t *testing.T) {
	...
	if !t.Run("doctor verifies the supported baseline", func(t *testing.T) {
		...
		wantOperations := []string{"auth.huggingface.login", "auth.huggingface.logout", "auth.huggingface.status", "boards.create", "boards.get", "boards.list", "generate", "generate", "generate", "images.get", "images.list", "images.upload", "models.install", "models.install", "models.list", "models.status", "queue.cancel", "queue.get", "queue.list", "queue.wait", "recall", "upscale", "upscale"}

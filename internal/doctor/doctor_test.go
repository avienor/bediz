...
func TestRunReportsReadinessForImplementedCapabilities(t *testing.T) {
	...
	wantOperations := []string{"models.list", "models.install", "models.install", "models.status", "images.list", "images.get", "images.upload", "queue.list", "queue.get", "queue.wait", "queue.cancel", "boards.list", "boards.get", "boards.create", "generate", "generate", "generate", "upscale", "upscale", "recall", "auth.huggingface.status", "auth.huggingface.login", "auth.huggingface.logout"}
...
func TestRunReportsInspectionAndUploadCapabilities(t *testing.T) {
	...
	for _, operation := range []string{"models.list", "models.install", "models.status", "images.list", "images.get", "images.upload", "queue.list", "queue.get", "queue.wait", "queue.cancel", "boards.list", "boards.get", "boards.create"} {
		if !got[operation] {
...
func TestRunAllowsReadOnlyInspectionOnEndpointCompatibleUntestedVersion(t *testing.T) {
	...
	if compatibility["models.install"] || compatibility["images.upload"] || compatibility["boards.create"] || compatibility["queue.cancel"] || compatibility["generate"] {

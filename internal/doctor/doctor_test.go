...
func TestRunReportsInspectionAndUploadCapabilities(t *testing.T) {
	...
	for _, operation := range []string{"models.list", "models.install", "models.status", "images.list", "images.get", "images.upload", "images.download", "queue.list", "queue.get", "queue.wait", "queue.cancel", "queue.clear", "boards.list", "boards.get", "boards.create"} {
		if !got[operation] {
